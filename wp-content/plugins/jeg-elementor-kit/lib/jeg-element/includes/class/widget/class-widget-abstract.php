...
abstract class Widget_Abstract extends \WP_Widget {
	...
	public function __construct() {
		...
		if ( apply_filters( 'jeg_widget_backend_script', is_admin() ) ) {
			...
			parent::__construct( $this->get_widget_id(), $instance->get_element_name(), array(
	...
	public function compatible_column() {
		...
		<div class="alert-element alert-info" style='margin-top: 15px;'>
			...
			<div class="alert-description"><?php esc_html_e( 'Please check style / design tab to change Module / Block width and make it fit with your current column width', 'jeg-element' ); ?></div>
		</div>
		<?php
	}
	...
	public function prepare_fields( $instance, $fields ) {
		...
		foreach ( $fields as $key => $field ) {
			...
			$setting[ $key ] = jeg_prepare_field( $key, $field, $instance, array(
	...
	public function widget( $args, $instance ) {
		...
		echo jeg_sanitize( $args['before_widget'] );

		if ( ! empty( $title ) ) {
			echo jeg_sanitize( $args['before_title'] ) . esc_html( $title ) . $args['after_title'];
		}

		echo jeg_sanitize( $this->get_view_instance()->render_widget( $instance ) );

		echo jeg_sanitize( $args['after_widget'] );
	}
